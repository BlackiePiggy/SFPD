...
def read_CN_value_from_obs_1D(SS_variables ,input_folder, output_folder, sat_type):
    ...
    for file in files_array:
        ...
        # 对每个 SS_variable 提取数据并按 code 保存
        for SS_variable in SS_variables:
            if SS_variable in obs_info.observation:
                # 创建一个字典来保存不同 code 的数据
                satellite_data = {}

                for entry in obs_info.observation[SS_variable].items():
                    timestamp, code = entry[0]
                    value = entry[1]
...
def read_CN_value_from_obs_allfile(SS_variables, satellite_code, input_folder, output_folder
                               ):
    def list_files_in_directory(directory_path):
        if not os.path.exists(directory_path):
            raise ValueError(f"Directory {directory_path} does not exist.")
