...
def read_el_from_sp3(station_name, inputdir, output_folder, satellite_code, station_llh, start_date_str, end_date_str):
    ...
    for file in os.listdir(inputdir):
        if file.endswith('.SP3'):
            ...
            outfilename = f'elevation_{station_name}_{file[11:18]}_{satellite_code}.csv'
            ...
            result = calculate_satellite_elevations(filename, station_llh, output_folder, satellite_code, full_path)
            print(f"Execution result for {outfilename}:", "Success" if result == 0 else "Failure")


def calculate_satellite_elevations(filename, station_llh, output_folder,satellite_code,full_path):
    """
    计算卫星仰角并将结果保存到CSV文件。
    ...
    try:
        ...
        sat_data = {satellite_code: sat_data[satellite_code]}
        ...
        for sat_id in sat_data:
            ...
            for i, epoch in enumerate(new_epochs):
                sat_x, sat_y, sat_z = sat_data[sat_id]['x'][i], sat_data[sat_id]['y'][i], sat_data[sat_id]['z'][i]
