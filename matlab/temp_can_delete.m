...
% 定义你要取的卫星号范围
satnos = {'G01', 'G03', 'G05', 'G06', 'G07', 'G08', 'G09', 'G10', 'G12', 'G15', 'G17', 'G24', 'G25', 'G26', 'G27', 'G29', 'G30', 'G31', 'G32'};
...
for i = 1:length(satnos)
    ...
    if isfile(full_path)
        ...
        % 打印两天的NaN值数量
        fprintf('Date %s has %d NaN values for satellite %s.\n', date_full1, sum(nan_idx1), satno);
        fprintf('Date %s has %d NaN values for satellite %s.\n', date_full2, sum(nan_idx2), satno);

        xlabel('Time (HH:MM:SS)');
        ylabel('Value');
        title(['Data for ', date_full1, ' and ', date_full2, ' - Satellite ', satno]);
        legend;
        grid on;
        hold off;
        
    else
        fprintf('File not found for satellite %s.\n', satno);
    end
end
